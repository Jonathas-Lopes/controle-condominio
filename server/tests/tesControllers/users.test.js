...
describe('Test userModel', () => {
    ...
    afterAll(async () => { await userModel.UserModel.deleteMany({ cpf: User.map((item) => item.cpf) })})
    it('should test insertUser', async () => {
        await userModel.insertUser(User)
        expect(User.map((item) => ({
            email: item.email,
            name: item.name,
            cpf: item.cpf,
            apt: item.apt,
            ...
            .toStrictEqual(await (await userModel.UserModel
                .find({})
                .lean())
                .map((item) => (
                    {
    ...
    it('should test delete a User', async () => {
        const userToDelete = Array.of(await userModel.UserModel.distinct('_id',{}));
